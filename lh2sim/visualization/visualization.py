...
def plot_pressures(
    ...
    if pressure_limits:
        if 'p_ET_high' in pressure_limits:
            ...
            ax.axhline(y=p_high, color='red', linestyle='--', 
                      linewidth=1, label=f'ET High Limit ({p_high:.2f} bar)')
        if 'p_ET_low' in pressure_limits:
            p_low = pressure_limits['p_ET_low'] / 1e5
            ax.axhline(y=p_low, color='orange', linestyle='--',
                      linewidth=1, label=f'ET Low Limit ({p_low:.2f} bar)')
...
def plot_summary_dashboard(
    ...
    if pressure_limits:
        if 'p_ET_high' in pressure_limits:
            ax2.axhline(y=pressure_limits['p_ET_high'] / 1e5, 
