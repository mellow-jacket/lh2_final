...
% Pressure settings
% For Scenario A (pump-driven): Tank 1 pressure decreases, minimum allowable 1.2 bar
% Tank 2 vents at ~10 bar to prevent over-pressurization
LH2Model.p_ST_low = 1.2*barToPa;        % [Pa] ST minimum allowable pressure (prevent air contamination)
LH2Model.p_ST_high = 1.3*barToPa;       % [Pa] ST vaporizer activation threshold (slight pressurization if too low)
LH2Model.p_ST_final = 3*barToPa;        % [Pa] final venting pressure for (ST) after transfer complete
...
LH2Model.p_ET_final = 10*barToPa;       % [Pa] final venting pressure for (ET) after transfer complete
